...
from src.contacts.repository import get_contacts, get_contact, create_contact, update_contact, delete_contact, \
    find_contacts, upcoming_birthday


class TestAsyncContacts(unittest.IsolatedAsyncioTestCase):

    def setUp(self) -> None:
        self.user = User(id=1, username="test_user", password="test_password", email="test_email", confirmed=True)
    ...
    async def test_get_contacts(self):
        ...
        contacts = [ContactModel(id=1, name="test_name", surname="test_surname", email="test_email", phone="test_phone",
                                 birthday="test_birthday", info="test_info", user=self.user),
    ...
    async def test_get_contact(self):
        limit = 10
        offset = 0
        contact = ContactModel(id=1, name="test_name", surname="test_surname", email="test_email", phone="test_phone",
                               birthday="test_birthday", info="test_info", user=self.user)
    ...
    async def test_create_contact(self):
        body = ContactSchema(name="test_name", surname="test_surname", email="test@email.com", phone="9876543210",
                             birthday="2020-04-01", info="test_info")
    ...
    async def test_update_contact(self):
        body = ContactUpdateSchema(name="test_name", surname="test_surname", email="test@email.com", phone="9876543210",
                                   birthday="2020-04-01", info="test_info")
        mocked_contact = MagicMock()
        mocked_contact.scalar_one_or_none.return_value = ContactModel(id=1, name="test_name", surname="test_surname",
                                                                      email="test_email", phone="9876543211",
                                                                      birthday="2020-04-01", info="test_info",
                                                                      user=self.user)
